...
export default class GameInstructionParser {
    ...
    private static async bepInExCorelibsPathResolver(game: Game, profile: Profile): Promise<string | R2Error> {
        try {
            ...
            const err: Error = e as Error;
            return new R2Error("Unable to resolver Corelibs folder", `"unstripped_corlib" folder failed. No such folder exists for path: ${Profile.getActiveProfile().getPathOfProfile()}.\nReason: ${err.message}`, null);
        }
    }
