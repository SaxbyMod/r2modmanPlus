...
export async function getPackageVersionNumbers(community: string, packageName: string) {
    const pkg = await getDexiePackage(community, packageName);
    return pkg.versions.map((v) => v.version_number);
}

/**
 * @param community Community which package listings should be used in the lookup.
...
export async function getVersionAsThunderstoreVersion(community: string, packageName: string, versionNumber: string) {
    const version = await getDexieVersion(community, packageName, versionNumber);
...
async function getDexiePackage(community: string, packageName: string) {
...
async function getDexieVersion(community: string, packageName: string, versionNumber: string) {
    const pkg = await getDexiePackage(community, packageName);
